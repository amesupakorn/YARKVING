...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-3 md:top-8 left-1/2 -translate-x-1/2 z-50 w-[95%] sm:w-[90%] max-w-5xl rounded-[1.5rem] md:rounded-full bg-surface-container-lowest/70 backdrop-blur-2xl shadow-ambient px-5 md:px-6 py-3 md:py-4 flex flex-col md:flex-row md:items-center justify-between border border-outline-variant/20 transition-all duration-300">
      <div className="flex items-center justify-between w-full md:w-auto">
        <Link href="/" className="flex items-center gap-2" onClick={() => setIsOpen(false)}>
          <img src="/image/icon.png" alt="YARKVING Logo" className="w-6 h-6 md:w-7 md:h-7 object-contain" />
          <span className="font-display font-semibold text-lg md:text-xl tracking-tight text-primary">YARKVING</span>
        </Link>
